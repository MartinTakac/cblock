...
{
public:
	CBlock_Autotrain_TestData(const char *testRunName, TestVariableWrapper goalIndex, const char *expectedName) : testRunName(testRunName), goalIndex(goalIndex), expectedName(expectedName)
	{
	}
...
public:
	static void SetUpTestCase()
	{
		...
		try
		{
			...
			RuntimeDataGenerator::generate_runtime_data(current_dir / "letters" / "cblock_letters_config.json",
														current_dir / "letters" / "runtime_data" / "scene" / "cblock",
														current_dir / ".." / "model_template",
														"RuntimeGenerator");
		}
		catch (exception &ex)
		{
			cerr << "Unable to generate runtime data for cblock_letters" << endl;
			exit(1);
		}
	}
	...
	virtual void SetUp()
	{
		...
		change_BL_directory("letters/runtime_data");
		srand(1);
	}

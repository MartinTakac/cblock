...
public:
	static void SetUpTestCase() {
		...
		try
		{
			// Note that the scene folder is not included in the template. Otherwise it would be harder
			// to drop into existing BL code
			RuntimeDataGenerator::generate_runtime_data(current_dir / "dialog" / "cblock_dialog_config.json",
														current_dir / "dialog" / "runtime_data" / "scene" / "cblock",
														current_dir / ".." / "model_template",
														"RuntimeGenerator");
		}
		catch (exception &ex)
		{
			cerr << "Unable to generate runtime data for cblock_bootstrap" << endl;
			exit(1);
		}
	}
...
TEST_F(cblock_bootstrap, Bootstrap) {
    ...
    runner.Run([](BL_model *model) {
        ...
        // Sequences for training - events are 3-hot, with events 919, 920, 921 indicating finalise sequence,
		// while event 922 resets sequence and clears state so that the next sequence starts with a blank state
...
public:
    static void SetUpTestCase() {
		...
		{
			cerr << "Unable to generate runtime data for cblock_playback_ngd" << endl;
			exit(1);
		}
    }
...
public:
    static void SetUpTestCase() {
		...
		{
			cerr << "Unable to generate runtime data for cblock_playback_ngd_digression" << endl;
			exit(1);
		}
    }
...
public:
    static void SetUpTestCase() {
		...
		{
			cerr << "Unable to generate runtime data for cblock_playback_gd" << endl;
			exit(1);
		}
    }
